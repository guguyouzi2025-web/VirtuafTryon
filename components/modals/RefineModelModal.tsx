...
interface RefineModelModalProps {
  modelImage: string;
  isOpen: boolean;
  onClose: () => void;
...
export const RefineModelModal: React.FC<RefineModelModalProps> = ({ modelImage, isOpen, onClose, onSave, onError }) => {
  const { t } = useI18n();
  ...
  useEffect(() => {
    ...
    img.onerror = () => {
        console.error("Failed to load image for refining model");
        onError("Failed to load image.");
    }
    img.src = `data:image/png;base64,${modelImage}`;

  }, [isOpen, modelImage, onError]);
  ...
  const handleApply = async () => {
    const mask = maskCanvasRef.current;
    if (!modelImage || !mask || !prompt) return;

    setIsProcessing(true);
    onError('');
    try {
        const maskBase64 = mask.toDataURL('image/png').split(',')[1];
        const result = await inpaintImage(modelImage, maskBase64, prompt);
        onSave(result);
    } catch (err) {
        console.error(err);
        onError(t('errors.refineModelFailed'));
    } finally {
  ...
  return (
    <div role="dialog" aria-modal="true" className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <h2 className="text-xl font-bold p-4 border-b border-gray-200 text-center">{t('refineModelModal.title')}</h2>
        ...
        <div className="p-4 border-t border-gray-200 bg-gray-50 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex-grow w-full md:w-auto">
             <label className="block text-sm font-medium text-gray-500 mb-1">{t('refineModelModal.promptLabel')}</label>
             <input 
                type="text" 
                value={prompt}
                onChange={e => setPrompt(e.target.value)}
                placeholder={t('refineModelModal.promptPlaceholder')}
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="primary" onClick={handleApply} disabled={!prompt || isProcessing}>
                {isProcessing ? t('buttons.applying') : t('buttons.saveChanges')}
            </Button>
          </div>
        </div>
      </div>
